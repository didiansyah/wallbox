...
export type VerificationResult = { status: VerificationStatus; expectedHash:string; actualHash:string; fileChecks:FileCheck[]; schemaErrors?:string[] };
...
export function verifyCapsule(capsule: unknown, expectedHash?: string): VerificationResult {
  ...
  if (!parsed.success) return { status:"INVALID_SCHEMA", expectedHash: expectedHash || "", actualHash:"", fileChecks:[], schemaErrors: parsed.error.issues.map(i => `${i.path.join(".")}: ${i.message}`) };
  const data = parsed.data;
  const expected = expectedHash || data.manifest.capsule_hash;
  const actualFiles = buildFileRecords({ trace:data.trace, sources:data.sources, verdict:data.verdict, artifacts:data.artifacts });
  const actualMap = new Map(actualFiles.map(f => [f.path, f]));
  const fileChecks = data.manifest.files.map((file) => {
    const actualFile = actualMap.get(file.path);
    if (!actualFile) return { path:file.path, status:"MISSING" as const, expectedSha256:file.sha256 };
    return { path:file.path, status: actualFile.sha256 === file.sha256 ? "OK" as const : "MISMATCH" as const, sha256: actualFile.sha256, expectedSha256:file.sha256 };
  });
