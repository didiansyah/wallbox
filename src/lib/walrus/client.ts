...
export function parseWalrusUploadResponse(response: unknown, fallback: string) {
  const obj = response as Record<string, any>;
  const blobId =
    obj?.blobId ||
    obj?.blob_id ||
    obj?.newlyCreated?.blobObject?.blobId ||
    obj?.newlyCreated?.blobObject?.blob_id ||
    obj?.alreadyCertified?.blobId ||
    obj?.alreadyCertified?.blob_id ||
    obj?.alreadyCertified?.event?.blobId ||
    obj?.alreadyCertified?.event?.blob_id;
  return String(blobId || fallback);
}

async function parseCapsuleResponse(res: Response): Promise<AuditCapsule> {
  const text = await res.text();
  try {
    return JSON.parse(text) as AuditCapsule;
  } catch {
    throw new Error(`INVALID_SCHEMA: Walrus aggregator returned non-JSON content (${res.headers.get("content-type") || "unknown"})`);
  }
}
...
export async function uploadCapsule(capsule: AuditCapsule): Promise<UploadResult> {
  ...
  return {
    blobId: `local-walrus-${capsule.manifest.run_id}-${capsule.manifest.capsule_hash.slice(2, 10)}`,
    mode: "local",
    network: "local-demo",
    note: "Local blob fallback uses the same interface as Walrus for hackathon development.",
  };
}
...
export async function fetchCapsule(blobId: string): Promise<AuditCapsule> {
  ...
  for (const delay of [0, 1000, 2000, 4000]) {
    ...
    if (res.ok) return parseCapsuleResponse(res);
